...
def getData():
    userInput = input("Enter Tweet term: ")
    print("fetching")
    pastSearches = CACHE_DICTION.keys()
    if userInput in pastSearches:
        print('using cache')
        return CACHE_DICTION[userInput]
    if userInput not in pastSearches:
        statuses = api.search(q = userInput)
        CACHE_DICTION[userInput] = statuses
        upload = json.dumps(CACHE_DICTION)
        cacheFile = open(CACHE_FNAME, "w")
        cacheFile.write(upload)
        cacheFile.close()
        return CACHE_DICTION[userInput]
...
while 1 < 3:
    data = getData()
    for x in range(5):
        text = data['statuses'][x]['text']
        created = data['statuses'][x]['created_at']
        print('TEXT: ' + text + '\n' + 'CREATED AT: ' + created + '\n')
    i += 1
